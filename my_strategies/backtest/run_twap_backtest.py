"""TWAP backtest runner — executes rebalance orders via TWAP (15min horizon, 1min intervals)."""
...
from twap_strategy import MyTWAPStrategy
from twap_strategy import MyTWAPStrategyConfig
...
def run_twap_backtest(
    ...
    engine_config = BacktestEngineConfig(
        ...
        logging=LoggingConfig(
            ...
            log_file_name=f"backtest_twap_{venue_key}_{timestamp}",
        ),
    )
    ...
    print(f"\nLogs written to: {log_dir}/backtest_twap_{venue_key}_{timestamp}.log")
    ...
    daily_snapshots_path = log_dir / f"daily_positions_twap_{timestamp}.csv"
    ...
    tearsheet_path = log_dir / f"tearsheet_twap_{venue_key}_{timestamp}.html"
    ...
    title = (
        ...
        f"TWAP: {config.twap_horizon_secs / 60:.0f}min horizon, {config.twap_interval_secs:.0f}s interval"
    )
    create_tearsheet(
        engine,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="1-MINUTE",
        help="Bar spec string, e.g. '1-MINUTE' (default: 1-MINUTE)",
    )
    parser.add_argument(
        "--external-data-dir",
        type=Path,
