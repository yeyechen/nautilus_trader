...
class MyTWAPStrategyConfig(StrategyConfig, frozen=True):
...
class MyTWAPStrategy(Strategy):

    def __init__(self, config: MyTWAPStrategyConfig):
    ...
    def on_start(self):
        ...
        self.log.info(
            f"Strategy started (TWAP) - first rebalance at {first_rebalance}, "
            f"horizon={self.config.twap_horizon_secs}s, interval={self.config.twap_interval_secs}s",
