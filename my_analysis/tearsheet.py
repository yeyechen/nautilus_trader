...
def _derive_daily_returns_from_equity(engine) -> pd.Series:
    ...
    if dollar_equity is None or dollar_equity.empty or len(dollar_equity) < 2:
        return pd.Series(dtype=float)

    # Resample to daily: take the last equity value per day (end-of-day balance)
    daily_equity = dollar_equity.resample("D").last().dropna()
